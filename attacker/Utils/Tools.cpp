...
int Tools::setNetworkParams() {
	...
	wsprintfA(timewaitcmd, timewaitcmdformat.c_str(), 30);
	...
	char kpalive[1024];
	wsprintfA(kpalive, kpaliveformat.c_str(), SELECT_TIME_OUT *100);
	ret = system(kpalive);
	...
	wsprintfA(kpaliveinternalcmd, kpaliveintervalformat.c_str(), SELECT_TIME_OUT * 100);
...
int Tools::openPortInFW(unsigned int port, string name, string protocol) {
