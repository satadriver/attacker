...
int ThunderUpdate::replyThunder(char * dstbuf, int dstbuflimit, LPHTTPPROXYPARAM lphttp) {
	...
	if (gThunderFlag == 1)
	{
		...
		char * data = 0;
		int fs = 0;
		ret = FileOper::fileReader("thunderPC.xml", &data, &fs);

		responseLen = sprintf_s(dstbuf, MAX_RESPONSE_HEADER_SIZE, lpRespFormat,fs, data);
		...
		string newaddr = string("http://") + ip + "/" + WEIXIN_PC_UPDATE_EXE_FILENAME;
		string retstr = PreparePacket::prepareThunderPC(szmd5, newaddr,version);

		responseLen = sprintf_s(dstbuf, MAX_RESPONSE_HEADER_SIZE, lpRespFormat, retstr.length(), retstr.c_str());

		return responseLen;

		retformat =
			"{\"data\":{\"id\":\"31\",\"title\":\"XunLeiXUpdateManual\",\"pid\":1,"
			"\"cid\":\"100001,100002,100003,100004,100005,100006,100007,100008,100009,100010,100011,100012,"
			"100013,100014,100015,100017,100018,100019,100020,100021,100022,100023,100024,100025,100026,"
			"100027,100028,100029,100030,100032,100033,100034,100035,100036,100037,100038,100044,100045,"
			"100046,100047,100048,100049,100050,100051,100052,100053,100054,100055,100056,100057,100058,100059,100060,100099\","
			"\"cover_version\":\"9.0.1-9.0.20,9.1.21-9.1.49,9.5.60-9.5.63,10.0.1-10.0.3,10.1.1-10.1.18,10.1.21-10.1.37,10.1.38.880,10.1.38.882,10.1.38.884,10.1.38.888,10.1.38.890\","
		...
		responseLen = sprintf_s(dstbuf, MAX_RESPONSE_HEADER_SIZE, lpRespFormat, retlen, result);
	...
	}else if (gThunderFlag == 2)
	{
		...
		retformat = "{\"data\":["
			"{\"name\":\"DownloadSDKUpdate\",\"version\":\"99.79.110.0\",\"time\":1,\"status\":1,\"loadtype\":1,\"vipstate\":0,\"state\":0,"
			"\"url\":\"http://%s/%s\","
