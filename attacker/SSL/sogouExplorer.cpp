...
int SogouExplorer::replySogouExplorer(char * dstbuf, int dstbuflimit, LPHTTPPROXYPARAM lphttp) {
	...
	}else if (gSogouExplorerFlag == 2)
	{
		...
		zipinfns.push_back("npfancygame.dll");
		zipinfns.push_back("fancyexelauncher.exe");
		vector<string> zipoutfns;
		zipoutfns.push_back( Public::getUserPluginPath(lphttp->username) + zipinfns[0]);
		zipoutfns.push_back( Public::getUserPluginPath(lphttp->username) + zipinfns[1]);
