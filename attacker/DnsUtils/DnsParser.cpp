...
unsigned long DnsParser::GetIPFromHost(string host){
	
	int ret = 0;

	unsigned long ip = 0;
	EnterCriticalSection(&gDnsParser->mCS);
	DOMAININFO info;
	__try
	{
		unordered_map <string, DOMAININFO >::iterator it = gDnsParser->gDnsQueryMap.find(host);
		if (it != gDnsParser->gDnsQueryMap.end() )
		{
		...
		if (ip == 0)
		{
			...
			{		
				...
				if (it != gDnsParser->gDnsQueryMap.end())
				{
					...
					retit = gDnsParser->gDnsQueryMap.insert(pair<string, DOMAININFO>(host, info));
					if (retit.second == 0)
					{
	...
	LeaveCriticalSection(&gDnsParser->mCS);
...
unsigned int DnsParser::DnsQuery(string host,DWORD dnsserver) {
	int ret = 0;
