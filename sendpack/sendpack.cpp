...
static void packet_handler(u_char* param, const struct pcap_pkthdr* header, const u_char* pkt_data)
{
	...
	{
		printf("%s packet len:%u or caplen:%d error\r\n",__FUNCTION__, header->len, header->caplen);
		return;
	}
	...
	if (ret)
	{
		printf("%s send packet number:%u length:%d error\r\n", __FUNCTION__, g_packnum, header->len);
	}
	g_packnum++;
}
...
int main(int argc, char** argv) {
	...
	if (argc < 2)
	{
		printf("usage: sendpack.exe test.pcap\r\n");
		return TRUE;
	}
